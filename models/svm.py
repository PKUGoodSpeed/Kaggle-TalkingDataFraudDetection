...
def Shaocong(train_file, valid_file, test_file, output_dir):
    ...
    print("Start Fitting ...")
    svc.fit(train_df[nn_predictors], train_df[Target])

    del train_df
    gc.collect()

    print("Predicting...")
    test_df['pred'] = svc.predict_proba(test_df[nn_predictors])[:, 1]
    ...
    print("Making OOF ...")
    valid_df['pred'] = svc.predict_proba(valid_df[nn_predictors])[:, 1]
